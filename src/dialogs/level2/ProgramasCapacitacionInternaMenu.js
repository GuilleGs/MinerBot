 ...
 * Presenta opciones de información sobre los programas de formación de la empresa.
 * Este es un diálogo informativo final en su rama de navegación.
 */
class ProgramasCapacitacionInternaMenu {
    ...
    constructor(bot) {
        this.bot = bot;
        // Normalizamos las claves para asegurar la coincidencia con content.js
        this.options = [
            'Capacitación DCL (Desarrollo de Competencias de Liderazgo)',
            'Capacitación en SSO (Seguridad y Salud Ocupacional)',
    ...
    async handleInput(context, text, conversationData, bot) {
        ...
        if (selectedOption) {
            ...
            // Normalizamos la clave para que coincida con content.js, que no tiene paréntesis.
            const contentKey = selectedOption.toLowerCase().replace(/\s*\(.*\)\s*/g, ' ').trim();
